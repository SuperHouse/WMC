			...
			DEVPROPKEY propKey)
		{
			...
			try
			{
				...
				if (errcode == ERROR_INVALID_DATA)
                    return "";
				retval = Encoding.Unicode.GetString(lbuffer);
			}
			finally
			{
            ...
            try
            {
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    if (Marshal.GetLastWin32Error() != ERROR_NO_MORE_ITEMS)
                    {
						...
						if(hardwareids!=""){
                            if (hardwareids.Length >= 21 && (hardwareids.Substring(0, 21) == "USB\\VID_03EB&PID_5740" || 
...
/*			if(devices.Count==0){
        ...
        static public List<USBDeviceInfo> GetFTDIDevices()
        {
            List<USBDeviceInfo> devices = new List<USBDeviceInfo>();
            ...
            try
            {
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    if (Marshal.GetLastWin32Error() != ERROR_NO_MORE_ITEMS)
                    {
                        ...
                        if (hardwareids != "")
                        {
                            if (hardwareids.Length >= 33 && hardwareids.Substring(0, 33) == "FTDIBUS\\COMPORT&VID_0403&PID_6001")
                            {
                                string busdescription = GetStringPropertyForDevice(info,
                                                         devdata, DEVPKEY_Device_BusReportedDeviceDesc);
                                string friendly = GetStringPropertyForDevice(info, devdata, DEVPKEY_Device_FriendlyName);
